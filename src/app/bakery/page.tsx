...
async function descargarLibro(
  ...
  const t = filas.reduce(
    (acc, d) => ({
      ...
      lib: acc.lib + d.comision_libre,
    }),
    { dep: 0, apo: 0, pag: 0, ret: 0, gan: 0, par: 0, rul: 0, mon: 0, cab: 0, lib: 0 }
  );
...
function GananciaPorJuego({ metricas }: { metricas: AdminMetricas | null }) {
  const fuentes = metricas
    ? [
        ...
        {
          nombre: "Ruleta libre",
          detalle: "Las que abren los jugadores",
          hoy: metricas.ganancia_libre_hoy,
          total: metricas.ganancia_libre_total,
        },
        {
          nombre: "Caballitos",
