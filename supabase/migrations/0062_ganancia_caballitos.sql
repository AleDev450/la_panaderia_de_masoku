-- =============================================================================
-- CACHUDOBET — Los caballitos, separados en el reporte de ganancias
-- 0062_ganancia_caballitos.sql
--
-- No es que faltaran: como los caballitos son rondas de `ruleta_rondas` con
-- `modo = 'carrera'` (0058), su comisión YA estaba contada en el total. El
-- problema es que estaba mezclada dentro de "Ruleta", así que el panel no
-- podía responder cuál de los dos juegos está trayendo la plata — que es justo
-- para lo que se hizo el desglose en 0052.
--
-- Ahora `ganancia_ruleta` cuenta solo `modo = 'ruleta'` y los caballitos van
-- en su propia columna. El TOTAL no cambia: lo que estaba en una fila ahora
-- está repartido en dos.
--
-- OJO CON LAS RONDAS CANCELADAS (0061): no aportan nada porque su
-- `comision_monto` queda en null y nunca se giraron. Se filtran igual por
-- `girada_at`, que solo se llena al sortear.
-- =============================================================================
...
returns table (
  ...
  -- Nuevo en 0062. Antes venía sumado dentro de `ganancia_ruleta_*`.
...
begin
  ...
  -- Las dos mitades de `ruleta_rondas`, separadas por `modo`.
  select coalesce(sum(r.comision_monto) filter (
    ...
    into v_ruleta_hoy, v_ruleta_total, v_caballos_hoy, v_caballos_total
  ...
  select
    ...
    -- Los caballitos entran en el total igual que antes: solo cambia que ahora
    -- se pueden mirar aparte.
    (v_partidas_hoy + v_ruleta_hoy + v_caballos_hoy + v_moneda_hoy - v_personal_hoy)::numeric,
    (v_partidas_total + v_ruleta_total + v_caballos_total + v_moneda_total
      - v_personal_total)::numeric,
...
-- ---------------------------------------------------------------------------
-- admin_resumen_diario: misma separación en el cuadro día a día.
--
-- Si el titular distingue los dos juegos y el cuadro de abajo los sigue
-- mezclando, no hay forma de cuadrar uno contra otro.
-- ---------------------------------------------------------------------------
  ...
  filas as (
    select
      ...
      coalesce(com.monto, 0) + coalesce(rul.monto, 0) + coalesce(mon.monto, 0)
        + coalesce(cab.monto, 0) as comision,
      coalesce(com.monto, 0) + coalesce(rul.monto, 0) + coalesce(mon.monto, 0)
        + coalesce(cab.monto, 0) - coalesce(pmg.monto, 0) as ganancia_real,
